...
public class MyWebSecurityConfigureAdapter extends WebSecurityConfigurerAdapter{
	private static final Logger logger = LoggerFactory.getLogger(MyWebSecurityConfigureAdapter.class);
	...
	private String encode(String rawPassword) {
		...
		logger.info("--Chris --> encode({}) with useBCryptPasswordEncoder2encodePassword:{} returning password as {}", rawPassword, useBCryptPasswordEncoder2encodePassword, password);
		return password;
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.antMatchers("/rest/v1/dogs/**").hasRole("dogMaster")
	...
	public void configureGlobal_(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication()
				...
				.and()
				.withUser("developer").password(encode("developer")).roles("ADMIN", "DEVELOPER","catMaster","dogMaster").authorities("haveDogs")
				.and()
